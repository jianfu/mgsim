...
void Processor::FamilyTable::Cmd_Read(ostream& out, const vector<string>& arguments) const
{
    ...
    {
        out << "    |     Initial PC     | Allocated | Threads | P/A/D/Rd |  Cores  | Link |    Sync    |     Capability     | State         |   Symbol  | redundant | threadCount | corr_fid" << endl;
        out << "----+--------------------+-----------+---------+----------+---------+------+------------+--------------------+---------------+-----------+-----------+-------------+----------" << endl;
        for (set<LFID>::const_iterator p = fids.begin(); p != fids.end(); ++p)
        {
            ...
            else
            {
				...
				out << left << setw (6) << setfill(' ') << family.redundant 
					<< " | " << left << setw(11) << setfill(' ') << family.threadCount
